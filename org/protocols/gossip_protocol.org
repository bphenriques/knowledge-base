...
#+EXPORT_FILE_NAME: protocols_gossip-protocol
#+ROAM_TAGS: protocols
#+ROAM_ALIAS:

Some of these text may be interwined with specifities of the Gossip Protocol within [[file:../uncategorized/akka.org][Akka]].
...
Each node is monitored for failures by at most 5 nodes using [[file:heartbeat.org][Heartbeat]]. Once a member is deemed unreachable, then than information will be gossiped to the cluster. The member will remain like that until all nodes flag the node as ~Reachabe~ again.
